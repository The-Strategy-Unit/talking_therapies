...
subtitle: "Evaluation of interventions to improve adherence to therapy in *NHS North Yorkshire Talking Therapies*"
author: "Craig Parylo"
date: today
...
params <- list(
  "ods_intervention" = "RX301",
  "intervention_long_name" = "NHS North Yorkshire Talking Therapies",
  "intervention_short_name" = "NHS North Yorkshire TT",
...
p <-
  ...
  zoo::scale_x_yearmon(limits = c(zoo::as.yearmon(c("Dec 2019")), NA)) +
  ...
  ggplot2::labs(
    ...
    subtitle = stringr::str_wrap(
      glue::glue(
        "Referral discharges appear in the IAPT dataset from January 2022"
      ),
      60
    ),
    # x = "",
...
**Recommendation:** drop `m2_rate`, `m3_rate`, `m12_rate` and `m14_rate` from the matching process as there is little scope to improve balance on these variables.
...
df_prep |>
  ...
  dplyr::filter(
    value %in% c(0L, 1L),
    !variable %in% c("m2_rate", "m3_rate", "m12_rate", "m14_rate") # already excluded from the above process
  ) |>
...
**Recommendation:** drop `m8_rate` and `m16_rate` from the matching process as they are likely to result in perfect separation.
...
matching_vars <- setdiff(
  matching_vars,
  c("m2_rate", "m3_rate", "m12_rate", "m14_rate", "m8_rate", "m16_rate")
)
t <- get_vif_score_table(matching_vars = matching_vars, df = df_prep)
t
```

**Next step:** evaluate the impact of removing `m4_rate` from the matching model.
...
**Next step:** drop `m5_rate` from the matching model.
...
**Next step:** remove `o1_denom`from the matching model.
...
**Next step:** drop `m10_rate` from the matching model.
...
Following matching, all variables show improved balance between `r params$intervention_long_name` and the eight matched services, as evidenced by blue points being closer to zero than red points.

All four variables fall within the 0.2 Standardised Mean Difference (SMD) threshold.
...
#### Navigo Health and Social Care CIC (NQL)
...
Outcome 1: ❌ The trends are not parallel - the control has 0% most months
...
Outcome 1: ✔️ The trends are almost parallel - they are slightly convergent

Outcome 2: ✔️ The trends are almost parallel - they are slightly divergent
...
#### City Health Care Partnership CIC (NNF)

Outcome 1: ❌ The trends are not parallel - they are convergent

Outcome 2: ✔️ The trends are almost parallel - there is a hint of divergence
...
Outcome 1: ❌ The trends are almost parallel though the control displays much more variance

Outcome 2: ❌ The trends are almost parallel though the control displays much more variance
...
matches_parallel_ods <- c("RTF61", "RXA29", "RP7")
...
# get a df with the details for our matched services details
df_analysis <-
  df |>
  dplyr::filter(
    # limit to activity in scope
    dplyr::between(
      calc_month,
      zoo::as.yearmon("Jan 2022"),
...
# return any anomalous observations for this data
identify_anomalies(
  df = df_analysis,
  outcome = "o1_rate",
  iqr_alpha = 0.05,
...
-   One control service, RTF61, exhibited an abnormal spike to 100% performance in May 2024.

To avoid the risk of bias, this anomalous points should be excluded from the dataset.
...
# show a spaghetti plot for these services details
p <-
  plot_spaghetti_plot(
    df = df_analysis,
    str_outcome = "o2_rate",
...
# return any anomalous observations for these data
identify_anomalies(
  df = df_analysis,
  outcome = "o2_rate",
  iqr_alpha = 0.05,
...
# get a df excluding the problematic months for the controls in this set
df_analysis2 <-
  df_analysis |>
  dplyr::filter(
    !(ods_code == "RTF61" & calc_month %in% zoo::as.yearmon(c("May 2024")))
  )
...
# conduct did analysis on the matched service details
did <-
...
Neither result is statistically signficant.

The point estimates give the best single indicator of direction and magnitude:

-   Outcome 1: small increase of 0.95%

-   Outcome 2: moderate decrease of -2.932%
...
-   For outcome 1 the DiD estimate is increased slightly.

-   For outcome 2 the DiD estimate becomes slightly more negative.
...
-   For outcome 1 the DiD estimate becomes negative with a much wider confidence interval

-   For outcome 2 the DiD estimate remains around -3% but with a much wider confidence interval
...
Across the baseline and alternative models, none of the DiD estimates reached statistical significance. Outcome 1 showed small positive effects (ranging from -1.10% to 1.18%) with wide confidence intervals crossing zero. Outcome 2 consistently showed negative estimates (around -3%) but with confidence intervals includeing zero, indicating no robust evidence of an effect under any specification.
...
After matching, most variables show improved similarity (the blue points are nearer to 0 than the red points) and three variables are within the 0.2 SMD threshold.

The 10 matched services found using using five cut-points are:
...
# extract the matched services to a var ddddd
...
#### Midlands Partnership NHS Foundation Trust (RRE)

**Outcome 1:** ❌ The trends are not parallel - they are convergent

**Outcome 2:** ❌ The trends are not parallel - there is a hint of divergence
...
**Outcome 1:** ✔️ The trends are parallel - there is a hint of divergence
...
A total of 10 Talking Therapy services were matched with `r params$intervention_long_name` using *Coarsened Exact Matching (CEM)*. Of these, 5 exhibited pre-intervention trends that closely align with those of `r params$intervention_long_name` for both outcome measures.
...
# return any anomalous observations d
identify_anomalies(
  df = df_analysis,
  outcome = "o1_rate",
  iqr_alpha = 0.05,
...
# show a spaghetti plot for outcome 2 d
p <-
  plot_spaghetti_plot(
    df = df_analysis,
    str_outcome = "o2_rate",
...
df_analysis2 <-
  df_analysis |>
  dplyr::filter(
    !(ods_code == "R0B" & calc_month %in% zoo::as.yearmon(c("Jun 2025"))),
    !(ods_code == "RTF61" & calc_month %in% zoo::as.yearmon(c("May 2024"))),
...
-   Outcome 1 remains positively affected by the intervention and the effect size is slightly larger, though the confidence interval has increased which means the result is not statistically significant.

-   Outcome 2 remains negatively affected by the intervention, though the increased confidence interval increased which means the result is not statistically significant.

#### Using all CEM controls

What impact does using all 10 matched control services have on the findings?
...
Across the baseline and both alternative specifications, no statistically significant effects were observed:

- Outcome 1 estimates were small positive values (ranging from 0.27% to 2.43%) but with confidence intervals that crossed zero,

- Outcome 2 estimates were consistently negative (around -1% to -1.6%) yet also non-significant.

Overall, the results suggest no robust evidence of an effect for either outcome under any model.
...
did <-
  delegate_synthdid_analysis(
    ...
    .ods_treated = params$ods_intervention,
    str_treated = params$intervention_short_name,
    summary_spec = "Alternative 3 - using a combination of PSM and CEM matches",
    summary_outcome = "Outcome 1",
    str_outcome = "o1_rate",
    labs_title = "Outcome 1: discharges with 5+ treatment contacts"
  )
...
saveRDS(
  object = did_meta,
  file = here::here("data", "project", "did_estimates", "yorkshire.Rds")
)
```
