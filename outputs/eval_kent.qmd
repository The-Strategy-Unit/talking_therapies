...
params <- list(
  ...
  "zoo_intervention_month" = zoo::as.yearmon("Jan 2025")
)
...
-   Manual difference-in-differences (DiD) using matched services via Propensity Score Matching (PSM) and Coarsened Exact Matching (CEM)
...
-   Outcome 1 findings suggest a possible positive effect, wtih statistical significance observed only under CEM matching. Results from PSM and synthetic DiD point in the same direction but remain non-significant.

-   Outcome 2 estimates are small, inconsistent in direction and non-signficiant across all methods, indicating no clear evidence of an effect.

**Limitations**

-   Only one treated unit, leading to wide confidence intervals.
...
`r params$intervention_long_name` (ODS code: [`r params$ods_intervention`](https://www.odsdatasearchandexport.nhs.uk/?search=generalorg&query=E0W9O)) is a newly formed organisation that merged several Talking Therapies servcies across Kent and Medway. IAPT records show a marked increase in discharges started in early 2024, with numbers rising steadily each month.

This presents a challenge for the current evaluation, as the available data is significantly reduced compared to available records. Many other Talking Therapy services have data extending back to January 2022, offering a richer longitudinal view that is now partially obscured.
...
p <-
  ...
  ggplot2::annotate(
    geom = "segment",
    x = zoo::as.yearmon("Oct 2023"),
    xend = zoo::as.yearmon("Feb 2024"),
    y = 500,
    yend = 500,
    colour = "#5881c1",
  ...
  ggplot2::labs(
    ...
    subtitle = stringr::str_wrap(
      glue::glue(
        "The service began operating from October 2023 with routine referral dishcarges appearing in the IAPT dataset from early 2024"
      ),
      60
    ),
    # x = "",
...
To reconstruct a pre-intervention baseline, we examined the activities of the former Talking Therapies services that were consolidated into `r params$intervention_long_name`. These antecedent organisations were identified using the IAPT dataset, informed by a [2018 presentation](https://www.kentmedwaymentalhealth.nhs.uk/media/1508/kandm-talking-therapies.pdf) and by tracking Kent and Medway-based services that ceased reporting between February and May 2024.

```{r}
#| label: antecedent organisations
#| cache: true

# possible list of previous K&M IAPT providers (published in 2018):
# https://www.kentmedwaymentalhealth.nhs.uk/media/1508/kandm-talking-therapies.pdf

ods_interventions_prior <- c(
  "AJA", # Dover Counselling
  "NIW", # Faversham Counselling Services
  # IESO (multiple subcontracts - still ongoing, so not using)
  "NI382", # We Are With You (likely to be previously known as Insight, active in Kent & Mediway)
  "NDC16", # Medway Talking Therapies
  "8J766", # North Kent Mind
  # Psicon
  # Think Action
  "NKT", # University Medical Centre
  "NDC07", # Every Turn Services Ltd - active in Kent and Medway ICB, last entry Feb 2024.
  "NWC10" # VITA HEALTH GROUP:VITAMINDS KENT AND MEDWAY, last entry Mar 2024
)

df |>
  dplyr::select(ods_code, name) |>
  dplyr::distinct() |>
  dplyr::filter(ods_code %in% ods_interventions_prior) |>
  dplyr::arrange(ods_code) |>
  gt::gt() |>
  gt::tab_options(quarto.disable_processing = TRUE) |>
  gt::cols_label(ods_code = "ODS Code", name = "Service name")
```

By integrating discharge data from both the newly formed organisation and its predecessor services, we generate the following time series of discharge activity.

```{r}
#| label: project discharges time series 2
#| fig-height: 7
#| cache: true
...
p <-
  df |>
  dplyr::filter(
    ods_code %in% c(params$ods_intervention, ods_interventions_prior)
  ) |>
  # combine the activities for these services each month
  dplyr::summarise(
    o1_denom_discharges_count = sum(o1_denom_discharges_count, na.rm = TRUE),
    .by = c(calc_month)
  ) |>
  ggplot2::ggplot(
    mapping = ggplot2::aes(
...
However, this approach has limitations. The merger itself constitutes a significant intervention, likely accompanied by operation changes and shifts in service delivery, which may confound direct comparisons with historical data.

Given these constraints, the analysis will focus on the truncated dataset spanning July 2024 to September 2025.
...
df_temp <-
  ...
  dplyr::filter(
    ...
    dplyr::between(
      x = calc_month,
      left = zoo::as.yearmon("Jul 2024"),
      right = zoo::as.yearmon("Sep 2025")
    )
  )
...
## Using all matching variables - Dec 2024

The dataset is cut off at December 2024, the month immediately before `r params$intervention_short_name` launched its first intervention. This pre-intervention slice will serve as the basis for the matching process.
...
- `m10_rate` (M10 Proportion of contacts where there was a therapist present) 

- `m14_rate` (M14 Proportion of contacts conducted in English)

**Recommendation:** drop `m3_rate`, `m6_rate`, `m10_rate` and `m14_rate` from the matching process as there is little scope to improve balance on these variables.
...
df_prep |>
  ...
  dplyr::filter(
    value %in% c(0L, 1L),
    !variable %in% c("m3_rate", "m6_rate", "m10_rate", "m14_rate") # already excluded from the above process
  ) |>
...
**Recommendation:** drop `m16_rate` from the matching process as it is likely to result in perfect separation.
...
matching_vars <- setdiff(
  matching_vars,
  c("m3_rate", "m6_rate", "m10_rate", "m14_rate", "m16_rate")
)
t <- get_vif_score_table(matching_vars = matching_vars, df = df_prep)
t
```

**Next step:** evaluate the impact of removing `m2_rate` from the matching model.
...
matching_vars <- setdiff(
  matching_vars,
  c("m3_rate", "m6_rate", "m10_rate", "m14_rate", "m16_rate", "m2_rate")
)
t <- get_vif_score_table(matching_vars = matching_vars, df = df_prep)
t
```

**Next step:** remove `m9_rate` from the matching model.
...
**Next step:** drop `m8_rate` from the matching model.
...
**Next step:** remove `m13_rate`from the matching model.
...
**Next step:** drop `m15_rate` from the matching model.
...
## Propensity score matching - Dec 2024
...
Following matching, most variables show improved balance between `r params$intervention_long_name` and the eight matched services, as evidenced by blue points being closer to zero than red points. However, one variable (M17) became notably more imbalanced post-matching.

Three variables fall within the 0.2 Standardised Mean Difference (SMD) threshold, and a fifth (M17) is outside this boundary.
...
#### Gloucestershire Health and Care NHSFT (RTQ)
    ...
    .f = \(.x) {
      p <-
        compare_matches_preintervention(
          ...
          period_preintervention = zoo::as.yearmon(c("Jul 2024", "Dec 2024")),
          trendline = TRUE
        )
      return(p)
    }
...
#### Bromley Healthcare (NQV)

Outcome 1: ✔️ The trends are almost parallel - there is a hint of divergence but they are very similar
...
#### Midlands Partnership University NHSFT (RRE)
...
#### Self Help Services (Stockport) (T6G0K)
...
Outcome 1: ❌ The trends are not parallel - they interesected around September 2024
...
matches_parallel_ods <- c("RTQ", "NQV", "RWRD7")
...
df_analysis <-
  df |>
  dplyr::filter(
    # limit to activity between Jul 2024 and Sep 2025
    dplyr::between(
      calc_month,
      zoo::as.yearmon("Jul 2024"),
...
p <-
  plot_spaghetti_plot(
    ...
    str_subtitle = "NQV displays abnormal results in September 2025",
...
-   One control service, NQV, exhibited an abnormal dip to 0% performance in outcome 1 in September 2025.

This brief, but steep decline would bias the post-implementation estimates downward for the controls. To avoid this bias, the low-performance points should be excluded from the dataset.
...
One control service, NQV, exhibited an abnormal dip to 0% in September 2025.
...
df_analysis2 <-
  df_analysis |>
  dplyr::filter(
    !(ods_code == "NQV" & calc_month %in% zoo::as.yearmon(c("Sep 2025"))),
  )
...
-   Outcome 1: moderate increase around 3%

-   Outcome 2: small increase of 0.9%
...
people_benefit_o1 <- floor(total_discharges * (3.02 / 100))
people_benefit_o2 <- floor(total_discharges * (0.94 / 100))
```
...
Assuming the 0.94% gain is real, it translates to an additional `r people_benefit_o2` people achieving reliable recovery per year (based on `r params$intervention_long_name`'s annual discharge volume of `r scales::comma(total_discharges)` from June 2024 to May 2025).
:::
...
-   For outcome 1 the DiD estimate is two percentage points higher, but the confidence interval is also wider.

-   For outcome 2 the DiD estimate increases, but the confidence interval is also wider.

Sensitivity analyses relaxing the exclusion of anomalous months yielded larger point estimates for both outcomes, though with wider confidence intervals. These results suggest that the estimated effects sensitive to the inclusion of outlier data and remain statistically inconclusive.
...
-   For outcome 1 the DiD estimate increases by more than two percentage points and the confidence interval is widened.

-   For outcome 2 the DiD estimate doubles suggesting a stronger positive impact and the confidence interval is wider.

Expanding the control group to include all eight matched services yielded directionally consistent results for both outcomes. The estimated effect for outcome 1 and outcome 2 increases, though with a wider confidence interval in both cases, illustrating the greater uncertainty of including all controls. Neither result reached statistical significance, but the findings suggest the estimated effects are robust to control group specification.
...
The direction of effect remains consistent across all anlyses (main and both sensitivity checks), which lends som qualitative support to a positive impact.

However, the lack of statistical significance and the sensitivy of estimates to model specifications suggest that the findings should be interpreted with caution.

## Coarsened exact matching - Dec 2024
...
The 14 matched services found using using five cut-points are:
...
#### The Counselling Foundation HQ (NGR01)
...
```{r}
#| label: cem - counselling foundation
#| fig-height: 8
#| cache: true
#| eval: true
...
#### NHS Oxfordshire Talking Therapies (RNUDT)

**Outcome 1:** ❌ The trends are not parallel - they are divergent

**Outcome 2:** ❌ The trends are not parallel - they intersected around October 2024

```{r}
#| label: cem - oxfordshire
#| fig-height: 8
#| cache: true
#| eval: true
...
#### NHS Buckinghamshire Talking Therapies (RNUDV)
...
#### Lincolnshire Partnership University NHS Foundation Trust (RP7)
...
#### Cambridgeshire and Peterborough Mental Health Partnership TR HQ (RT119)
...
#### Humber Teaching NHSFT (RV9)

**Outcome 1:** ❌ The trends are not parallel - they intersected around October 2024
...
#### Avon and Wiltshire Mental Health Partnership NHS Trust (RVN)
...
#### Coventry and Warwickshire Partnership NHS Trust (RYG)
...
A total of 14 Talking Therapy services were matched with `r params$intervention_long_name` using *Coarsened Exact Matching (CEM)*. Of these, 7 exhibited pre-intervention trends that closely align with those of `r params$intervention_long_name` for both outcome measures.
...
Now we have matched `r params$intervention_long_name` with 7 other Talking Therapy services and assessed their likelihood of meeting the parallel trends assumption based on their pre-intervention data, let us review how the full time series looks for this group of services.
...
-   RP7 dipped to 0% in September 2025
...
-   RP7 performance rapidly rose to 100% in September 2025.
...
df_analysis2 <-
  df_analysis |>
  dplyr::filter(
    !(ods_code == "RP7" & calc_month %in% zoo::as.yearmon(c("Sep 2025")))
  )
...
For outcome 1, `r params$intervention_long_name`'s observed rate is about **4.43 higher** than what the matched controls would predict, with a 95% confidence interval (0.83% to 8.04%) that includes a **statistically significant difference**.

For outcome 2, `r params$intervention_long_name`'s rate is about **2.07% lower** than expected, with a 95% confidence interval (-5.28% to 1.15%) that includes zero, indicating **no statistically reliable difference**.
...
# work out the expected number of people to benefit from the uplift
people_benefit_o1 <- floor(total_discharges * (4.43 / 100))
people_benefit_o2 <- floor(total_discharges * (-2.07 / 100))
```

These figures clearly present `r params$intervention_long_name`'s post-intervention performance exceeding the counterfactual based on the matched services for outcome 1, which is statistically significant. This finding suggests the interventions had a positive causal impact on the proportion of people discharged with five or more treatment contacts.

Assuming the 4.43% gain is real, it translates to an additional `r people_benefit_o1` people achieving 5+ treatment contacts per year (based on `r params$intervention_long_name`'s annual discharge volume of `r scales::comma(total_discharges)` from June 2024 to May 2025).
:::
...
1.  **Parallel trend selection** - We inspected the 14 matched controls series visually and excluded 7 that did not follow `r params$intervention_long_name`'s pre-intervention trend.

2.  **Anomalous month removal** - For the remaining matched services we omitted a month for a service with extreme values, which yielded more stable performance across outcomes.
...
-   Outcome 1 remains significantly and positively affected by the intervention and the effect size is slightly larger

-   Outcome 2 remains negative but non-statistically significant, whilst the confidence interval became larger
...
-   Outcome 1 remains significantly and positively affected by the intervention within a similar confidence range.

-   Outcome 2 remains not statitistically significant as the confidence interval includes zero. The negative point estimate is smaller indicating a weaker potential effect when all CEM controls are included.
...
-   **Outcome 1** There was a consistent positive and statistically significant effect across all specifications.

-   **Outcome 2** There was a consistent negative effect which was not statistically significant.

This pattern suggests that outcome 1's significance is robust to model specification.
...
set.seed(12345)
.yearmon_period = zoo::as.yearmon(c("Jul 2024", "Sep 2025"))
...
`r params$intervention_long_name`'s orange line rises more sharply than the dotted counterfactual, indicating a positive impact of the interventions. The estimated gain is 3.2% above the expected value. The 95% confidence interval spans -6.6% to + 13%, meaning the true effect could be negative or positive. Consequently, the result is **not statistically significant**.
...
`r params$intervention_long_name`'s orange line does not rise as much as the dotted counterfactual, indicating a lower-than-expected effect in the post-intervention period. The estimated difference is 0.04% below the expected value. The 95% confidence interval spans -10.1% to +9.2%, meaning the true effect could be negative or positive. Consequently, the result is **not statistically significant**.
...
# work out the expected number of people to benefit from the uplift
people_benefit_o1 <- floor(total_discharges * (3.2 / 100))
people_benefit_o2 <- floor(total_discharges * (-0.4 / 100))
```

The charts indicate `r params$intervention_long_name`'s post-intervention performance exceeding the synthetic counterfactual for outcome 1, but the wide confidence intervals prevents a definitive claim of effectiveness for both outcomes.

Assuming the 3.2% gain for Outcome 1 is real, it translates to an additional `r people_benefit_o1` people receiving 5+ treatment contacts per year (based on `r params$intervention_long_name`'s annual discharge volume of `r scales::comma(total_discharges)` from June 2024 to May 2025).
:::
...
`r params$intervention_long_name` performed 6.8% above the synthetic control expectation, however, the 95% confidence interval (-3.6% to 17.1%) includes zero so we cannot make any definitive claim.
...
`r params$intervention_long_name` performed 1.6% above the synthetic control expectation, yet the 95% confidence interval (-21.7% to 24.9%) includes zero so we cannot make any definitive claim.
...
`r params$intervention_long_name`'s observed outcome was **6.5% higher** than the synthetic control expectation.yet the 95% confidence interval (-1.5% to 14.6%) includes zero so we cannot make any definitive claim.
...
`r params$intervention_long_name`'s observed outcome was **1.3% lower** than the synthetic control expectation. However, the 95% confidence interval (-10.4% to 7.8%) includes zero, so the result is not statistically distinguishable from no effect.

#### Using matches from both PSM and CEM

What impact does using the matches returned from the propensity score matching (PSM) and coarsened exact matching (CEM) section have?

```{r}
#| label: synthdid - sensitivity both - outcome 1
#| fig-height: 7
#| warning: false
#| cache: true
#| eval: true
...
df_synth <-
  prepare_df_for_synthdid(
    ...
    yearmon_intervention = .yearmon_intervention,
    ods_treated = params$ods_intervention,
    ods_controls = unique(c(controls$psm_all, controls$cem_all)),
    df_intervention_services = df_intervention_services
  )
...
did <-
  delegate_synthdid_analysis(
    ...
    .ods_treated = params$ods_intervention,
    str_treated = params$intervention_short_name,
    summary_spec = "Alternative 3 - PSM and CEM matches",
    summary_outcome = "Outcome 1",
    str_outcome = "o1_rate",
    labs_title = "Outcome 1: discharges with 5+ treatment contacts"
  )
...
did_meta <-
  dplyr::bind_rows(
    did_meta,
    tibble::tibble(
      specification = "Synthdid alternative 3 (PSM & CEM controls)",
      outcome = "Outcome 1",
      estimate = as.numeric(did$did_estimate),
      se = did$did_se
    )
...
```{r}
#| label: synthdid - sensitivity both - outcome 2
#| fig-height: 7
#| warning: false
#| cache: true
#| eval: true
...
saveRDS(
  object = did_meta,
  file = here::here("data", "project", "did_estimates", "kent.Rds")
)
```
...
supp1 <-
  delegate_supp_outcome_analysis(
    ...
    ods_controls = c("RAT"),
    zoo_intervention = params$zoo_intervention_month,
    zoo_period = zoo::as.yearmon(c("Jul 2024", "Sep 2025"))
  )
...
The bar chart below displays `r params$intervention_long_name`'s data only. The chart highlights a **1.9 percentage point decrease in mutually agreed discharges** among individuals who received five or more treatment contacts in the post-intervention period, compared to the pre-intervention period. This shift was accompanied by small increases across the remaining categories.
...
-   **Manual DiD** - `r params$intervention_long_name` was compared with 10 services selected through a propensity score matched (PSM) and coarsened exact matched (CEM) procedures
...
`r params$intervention_long_name` was a newly introduced service that brought together several previous separate Talking Therapy services. Reliable discharge activity began in July 2024, marking the start of a risking trend that peaked in July 2025. Because the commencement of interventions in `r params$zoo_intervention_month` occurred during this onboarding period, the observed changes in outcome variables may reflect transitional effects of service consolidation rather than the intervention itself.