...
subtitle: "Evaluation of interventions to improve adherence to therapy in *NHS Bradford District and Craven Talking Therapies*"
author: "Craig Parylo"
date: today
...
params <- list(
  "ods_intervention" = "TAD",
  "intervention_long_name" = "NHS Bradford District and Craven Talking Therapies",
  "intervention_short_name" = "NHS Bradford District & Craven TT",
...
p <-
  ...
  zoo::scale_x_yearmon(limits = c(zoo::as.yearmon(c("Jan 2020")), NA)) +
  ...
  ggplot2::labs(
    ...
    subtitle = stringr::str_wrap(
      glue::glue(
        "Referral dishcarges appear in the IAPT dataset from January 2022"
      ),
      60
    ),
    # x = "",
...
- `m2_rate` (M3 Proportion of discharges for people aged under 26 years at referral)

- `m14_rate` (M14 Proportion of contacts conducted in English)

- `m15_rate` (M15 Proportion of contacts with an interpreter present) 

**Recommendation:** drop `m2_rate`, `m14_rate` and `m15_rate` from the matching process as there is little scope to improve balance on these variables.
...
df_prep |>
  ...
  dplyr::filter(
    value %in% c(0L, 1L),
    !variable %in% c("m2_rate", "m14_rate", "m15_rate") # already excluded from the above process
  ) |>
...
**Recommendation:** drop `m11_rate` and `m16_rate` from the matching process as they are likely to result in perfect separation.
...
matching_vars <- setdiff(
  matching_vars,
  c("m2_rate", "m14_rate", "m15_rate", "m11_rate", "m16_rate")
)
...
matching_vars <- setdiff(
  matching_vars,
  c("m2_rate", "m14_rate", "m15_rate", "m11_rate", "m16_rate", "m12_rate")
)
t <- get_vif_score_table(matching_vars = matching_vars, df = df_prep)
t
```
...
**Next step:** remove `m10_rate` from the matching model.
...
These results indicate that the matched services are really close, though not perfect, comparators for `r params$intervention_long_name`.

Following matching, nearly all variables show improved balance between `r params$intervention_long_name` and the eight matched services, as evidenced by blue points being closer to zero than red points. However, one variable (M3) became slightly more imbalanced post-matching.

Five variables fall within the 0.2 Standardised Mean Difference (SMD) threshold, and a sixth (m3) is close to, but outside, this boundary.
...
#### Sunderland Mind (Z2E3K)

Outcome 1: ❌ The trends are not parallel - they intersected around January 2024

Outcome 2: ❌ The trends are not parallel - they are divergent

```{r}
#| label: PSM - sunderland
#| fig-height: 8
#| cache: true
#| eval: true
...
#### Self Help Services (Manchester) (AMC05)

Outcome 1: ✔️ The trends are almost parallel - there is a hint of convergence but they are very similar

Outcome 2: ❌ The trends are not parallel - they are convergent

```{r}
#| label: PSM - manchester
#| fig-height: 8
#| cache: true
#| eval: true
plot_list[[2]]
...
#### Pure Innovations Limited (8KJ43)
...
Outcome 2: ✔️ The trends are almost parallel - there is a hint of convergence
...
Outcome 1: ✔️ The trends are almost parallel - there is a hint of convergence
...
#### Grange Park Clinic Monkmouth Hospital Sunderland Locality (RX4K3)
...
#### Six Degrees Social Enterprise CIC (NCM)
...
matches_parallel_ods <- c("RQX", "RX4K3", "RXA29")
...
- The intervention service exhibited an abnormal drop to 0% performance in October 2023.

- One control service, RX4K3, exhibited an abnormal spike to 100% performance in April 2025.

To avoid the risk of bias, these anomalous points should be excluded from the dataset.
...
- Control service, RX4K3, exhibited an abnormal spike to 100% performance in April 2025.

- Control service, RQX, exhibited an abnormal drop to 27% performance in January 2025.
...
df_analysis2 <-
  df_analysis |>
  dplyr::filter(
    !(ods_code == "RX4K3" & calc_month %in% zoo::as.yearmon(c("Apr 2025"))),
    !(ods_code == "RQX" & calc_month %in% zoo::as.yearmon(c("Jan 2025"))),
    !(ods_code == "TAD" & calc_month %in% zoo::as.yearmon(c("Oct 2023")))
  )
...
There is a statistically significant result for outcome 1.

The point estimates give the best single indicator of direction and magnitude:

-   Outcome 1: strong increase of 7.27%

-   Outcome 2: moderate increase of 2%
...
- Assuming the `r scales::percent(did$estimate[1], accuracy = 0.01)` change is real, it translates to `r dplyr::if_else(did$estimate[1] > 0, "an additional", "a reduction of")` `r people_benefit_o1` people receiving 5+ treatment contacts as part of their referral each year.

- Assuming the `r scales::percent(did$estimate[2], accuracy = 0.01)` change is real, it translates to `r dplyr::if_else(did$estimate[2] > 0, "an additional", "a reduction of")` `r people_benefit_o2` people achieving reliable recovery each year.
...
# conduct did analysis on the  matched services
did <-
  get_manual_did_estimation(
    df = df_analysis,
    ods_intervention = params$ods_intervention,
...
-   For outcome 1 the DiD estimate is over a percentage point higher, with a wider confidence interval.

-   For outcome 2 the DiD estimate increases, but the confidence interval is also significantly wider.

Sensitivity analyses relaxing the exclusion of anomalous months yielded larger point estimates for both outcomes, though with wider confidence intervals. These results suggest that the estimated effects sensitive to the inclusion of outlier data.
...
df_analysis3 <-
  df |>
  dplyr::filter(
    # limit to activity within the time period
    dplyr::between(
      calc_month,
      zoo::as.yearmon("Jan 2022"),
      zoo::as.yearmon("Sep 2025")
...
-   For outcome 1 the DiD estimate increases slightly and the confidence interval is widened.

-   For outcome 2 the DiD estimate doubles and become statistically significant and the confidence interval is wider.

Expanding the control group to include all eight matched services yielded directionally consistent results for both outcomes. The estimated effect for outcome 1 and outcome 2 increases, though with a wider confidence interval in both cases, illustrating the greater uncertainty of including all controls. Both results reached statistical significance, and the findings suggest the estimated effect for outcome 1 is robust to control group specification.
...
The same matching variables selected from the multicollinearity optimisation process will be used. We use six cutpoints to divide each matching variable into seven separate bins.
...
# extract the matched services to a var dd
...
#### African and Caribbean Mental Health Services (8HX13)

**Outcome 1:** ❌ The trends are not parallel - the control has too few points and is much more variable that the intervention service

**Outcome 2:** ❌ The trends are not parallel - the control has too few points and is much more variable that the intervention service
...
#### Lancashire Women (AKR01)

**Outcome 1:** ✔️ The trends are parallel

**Outcome 2:** ❌ The trends are not parallel - they intersected around July 2023
...
#### Newham Talking Therapies (RWK79)
...
#### Rotherham Doncaster and South Humber NHS Foundation Trust (RXE)
...
A total of 8 Talking Therapy services were matched with `r params$intervention_long_name` using *Coarsened Exact Matching (CEM)*. Of these, 4 exhibited pre-intervention trends that closely align with those of `r params$intervention_long_name` for both outcome measures.
...
Now we have matched `r params$intervention_long_name` with 4 other Talking Therapy services and assessed their likelihood of meeting the parallel trends assumption based on their pre-intervention data, let us review how the full time series looks for this group of services.
...
df_analysis2 <-
  ...
  dplyr::filter(
    !(ods_code == "RQX" & calc_month %in% zoo::as.yearmon(c("Jan 2025"))),
    !(ods_code == "RWK79" & calc_month %in% zoo::as.yearmon(c("Sep 2023"))),
    !(ods_code == "RX4K3" & calc_month %in% zoo::as.yearmon(c("Apr 2025"))),
    !(ods_code == "TAD" & calc_month %in% zoo::as.yearmon(c("Oct 2023")))
  )
...
This table summarises the Difference-in-Differences analysis for `r params$intervention_long_name`'s interventions using the 4 controls identified using the Coarsened Exact Matching process and which exhibited parallel trends.

For outcome 1, `r params$intervention_long_name`'s observed rate is about **4.38 percentage points higher** than what the matched controls would predict, with a 95% confidence interval (0.39% to 8.37%) that indicates a **statistically significant difference**.

For outcome 2, `r params$intervention_long_name`'s rate is about **2.69 percentage points higher** than expected, with a 95% confidence interval (0.64% to 4.73%) that indicates a **statistically significant difference**.
...
1.  **Parallel trend selection** - We inspected the 8 matched controls series visually and excluded 4 that did not follow `r params$intervention_long_name`'s pre-intervention trend.
...
-   Outcome 1 remains positively affected by the intervention and the effect size is slightly larger, though the confidence interval has increased which means the result is no longer statistically significant.

-   Outcome 2 remains positively affected by the intervention and effect size is also slightly larger, though the confidence interval increased which means the result is no longer statistically signfiicant.
...
-   Outcome 2 remains significantly and positively affected by the intervention within a wider confidence range.
...
-   **Outcome 1** There was a consistently positive effect size measured which was statistically significant effect in the main model and the model using all CEM controls, but not when including anomalous months.

-   **Outcome 2** There was a consistently positive effect size measured which was statistically significant effect in the main model and the model using all CEM controls, but not when including anomalous months.
...
# get a df that handles the anomalous result in Oct 2023
df_temp <-
  df |>
  # remove the month's data
  dplyr::filter(
    !(ods_code == "TAD" & calc_month %in% zoo::as.yearmon(c("Oct 2023")))
  ) |>
  # interpolate the missing data
  interpolate_missing_month_outcomes_for_intervention_service(
    .ods_intervention = params$ods_intervention
  )
...
did <-
  delegate_synthdid_analysis(
    ...
    .yearmon_intervention = .yearmon_intervention,
    .yearmon_period = .yearmon_period,
    .yearmon_scale_to = zoo::as.yearmon("Nov 2026"),
    .ods_treated = params$ods_intervention,
...
# work out the total discharges in 12 months preceeding d
total_discharges <-
  df |>
  dplyr::filter(
    ods_code == params$ods_intervention,
...
`r params$intervention_long_name` performed 4.4% above the synthetic control expectation, and the 95% confidence interval (2% to 6.8%) indicates a statistically significant result.
...
`r params$intervention_long_name` performed 0.5% below the synthetic control expectation, yet the 95% confidence interval (-1.4% to 0.5%) includes zero so we cannot make any definitive claim.
...
`r params$intervention_long_name`'s observed outcome was **4.2% higher** than the synthetic control expectation and the 95% confidence interval (4.1% to 4.3%) indicates a significant result.
...
`r params$intervention_long_name`'s observed outcome was **1.1% higher** than the synthetic control expectation, and the 95% confidence interval (0.3% to 1.9%) indicates this is a signficant result.
...
saveRDS(
  object = did_meta,
  file = here::here("data", "project", "did_estimates", "bradford.Rds")
)
```

Outcome 1 returned statistically significant results in both alternative models, with outcome 2 being statistically significant only when including CEM-matched controls.

These findings indicate the main model's conclusions are sensitive to changes in model specification.
