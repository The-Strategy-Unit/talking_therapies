...
subtitle: "Evaluation of interventions to improve adherence to therapy in *NHS Cornwall and Isles of Scilly Talking Therapies*"
author: "Craig Parylo"
date: today
...
params <- list(
  "ods_intervention" = "RJ8",
  "intervention_long_name" = "NHS Cornwall and Isles of Scilly Talking Therapies",
  "intervention_short_name" = "NHS Cornwall and IoS TT",
  "download_from_teams" = FALSE,
  "zoo_intervention_month" = zoo::as.yearmon("Jan 2023")
)
...
-   Outcome 2 showed positive effects in some models, but results were highly sensitive to analytical choices.

-   Outcome 1 showed a mixture of slightly positive trends, but none reached statistical significance.
...
A patient portal launched in September 2025 enables individuals to book and reschedule their assessment appointments. However, the current IAPT data cannot yet evaluate the impact of this recent intervention.
...
matching_vars <- setdiff(
  matching_vars,
  c("m14_rate", "m15_rate", "m16_rate")
)
t <- get_vif_score_table(matching_vars = matching_vars, df = df_prep)
t
```

**Next step:** evaluate the impact of removing `m9_rate` from the matching model.
...
matching_vars <- setdiff(
  matching_vars,
  c("m14_rate", "m15_rate", "m16_rate", "m9_rate")
)
t <- get_vif_score_table(matching_vars = matching_vars, df = df_prep)
t
```
...
matching_vars <- setdiff(
  matching_vars,
  c("m14_rate", "m15_rate", "m16_rate", "m9_rate", "m3_rate")
)
t <- get_vif_score_table(matching_vars = matching_vars, df = df_prep)
t
```
...
**Next step:** drop `m5_rate` from the matching model bacause it is still not converge.
...
Following matching, most variables show improved balance between `r params$intervention_long_name` and the eight matched services, as evidenced by blue points being closer to zero than red points. However, one variable (M1) became more imbalanced post-matching.

Four variables fall within the 0.2 Standardised Mean Difference (SMD) threshold, and a fifth (M1) is just outside this boundary.
...
Outcome 2: ✔️ The trends are almost parallel, there is a sign of convergence

```{r}
#| label: PSM - Denton House
#| fig-height: 8
#| cache: true
    ...
    .f = \(.x) {
      p <-
        compare_matches_preintervention(
          ...
          period_preintervention = zoo::as.yearmon(c("Jan 2022", "Dec 2022")),
          trendline = TRUE
        )
      return(p)
    }
...
```{r}
#| label: PSM - Herefordshire and Worcestershire
#| fig-height: 8
#| cache: true
plot_list[[2]]
```

#### Tees, Esk, Wear Valley NHS Trust (Durham) (RX301)
...
#### First Step IAPT (RX40C)

Outcome 1: ❌ The trends are not parallel - they are slightly convergent

Outcome 2: ❌ The trends are not parallel - they are slightly convergent

```{r}
#| label: PSM - First Step
#| fig-height: 8
#| cache: true
plot_list[[4]]
```
...
Outcome 1: ✔️ The trends are parallel, there is a fluctuation but otherwise looks good

Outcome 2: ✔️ The trends are parallel, there is a fluctuation but otherwise looks good

```{r}
#| label: PSM - midlands partnership
#| fig-height: 8
#| cache: true
plot_list[[5]]
```

#### Humber Teaching NHS Foundation Trust (RV9)

Outcome 1: ❌ The trends are not parallel, additionally there is wide fluctuation in the control's series

Outcome 2: ❌ The trendlines appear to be almost parallel, however there is too much fluctation in the control time series to be confident
...
#### Black Country Healthcare NHS Foundation Trust (TAJ)

Outcome 1: ✔️ The trends are almost parallel, there is slight convergence but otherwise looks a good match

Outcome 2: ✔️ The trends are almost parallel

```{r}
#| label: PSM - Black Country
#| fig-height: 8
#| cache: true
plot_list[[8]]
```
...
A total of eight Talking Therapy services were matched with `r params$intervention_long_name` using *Propensity Score Matching (PSM)*. Of these, two had pre-intervention trends that visually match those of `r params$intervention_long_name`'s for both outcome measures.
...
matches_parallel_ods <- c("RRE", "TAJ")
...
Now we have matched `r params$intervention_long_name` with two other Talking Therapies services and assessed their likelihood of meeting the parallel trends assumption based on their pre-intervention data, let us review how the full time series looks for this group of services.
...
df_analysis <-
  ...
  dplyr::filter(
    ...
    ods_code %in% c(params$ods_intervention, controls$psm_parallel)
  ) |>
  # simplify the data
...
-   There are two time series displayed, one for `r params$intervention_long_name`, shown in orange, and one for each of the two of our matched services, shown in grey.

-   Each series is identified by its ODS code at the end of the data in August 2025.
...
This table summarises the Difference-in-Differences (DiD) analysis for `r params$intervention_long_name`'s interventions using the two control services identified using the Propensity Score Matching process and which exhibited parallel trends.

This table summarises the Difference-in-Differences analysis for Cornwall’s interventions using the single control identified using the Propensity Score Matching process and which exhibited parallel trends.

For outcome 1, Cornwall’s observed rate is about 0.82% higher than what the matched controls would predict, yet the 95% confidence interval (-2.55% to 4.19%) includes zero, indicating no statistically reliable difference.

For outcome 2, Cornwall’s rate is about 4.87% higher than what the matched control would predict, and the 95% confidence interval (2.7% to 7.05%) indicates the effect is statistically significant.
...
people_benefit_o1 <- floor(total_discharges * (0.82 / 100))
people_benefit_o2 <- floor(total_discharges * (4.87 / 100))
```

These findings indicate `r params$intervention_long_name`'s post-intervention performance exceeds the counterfactual for both outcomes, but the wide confidence interval in Outcome 1 is too wide to support a definitive claim of effectiveness meaning only Outcome 2 is statistically significant.

This means we have evidence that Cornwall’s rate of people being discharged with reliable recovery is higher than expected, and this improvement cannot be wholly explained by a larger proportion of clients receiving five or more therapeutic contacts during referral.

Assuming the 4.87% gain is real, it translates to an additional `r people_benefit_o2` people achieving reliable recovery per year (based on `r params$intervention_long_name`'s annual discharge volume of `r scales::comma(total_discharges)` from June 2024 to May 2025).
:::
...
1.  **Parallel trend selection** - We inspected the 8 matched control services visually and excluded six that did not follow `r params$intervention_long_name`'s pre-intervention trend.
...
-   For outcome 1 the DiD estimate becomes slightly more positive and the confidence interval is narrowed.

-   For outcome 2 the DiD estimate increases suggesting a stronger positive impact and the confidence interval is narrowed.

Expanding the control group to include all eight matched services yielded directionally consistent results for both outcomes. The estimated effect for outcome 1 became slightly more positive, while outcome 2 showed a stronger positive effect.

These results imply that while there may be small effects, **there is insufficient evidence to conclude a meaningful impact** under all PSM controls.

## Coarsened exact matching - Dec 2022
...
Applying four cut-points allowed the CEM procedure to match `r params$intervention_long_name` with 6 Talking Therapy services. When we increased the number of cut-points to five - intended to boost sensitivity - only one matches were found.
...
We now assess the parallel trends for `r params$intervention_long_name` (shown in orange) and each of the 6 matched Talking Therapy services (shown in grey). Trendlines are overlaid on each plot to make it easier to judge whether the series move together.

#### Vita Health Group Newcastle IAPT (C2J4D)

**Outcome 1:** ❌ The trends are not parallel - they intersected in late 2022

**Outcome 2:** ✔️ The trends are almost parallel,there is a hint of divergent but overall a close match

```{r}
#| label: cem - Vita Health Newcastle
#| fig-height: 8
#| cache: true
#| eval: true
...
#### Alliance Psychology Services LTD (NJG)

**Outcome 1:** ❌ The trends are not parallel - they are convergent

**Outcome 2:** ✔️ The trends are almost parallel,there is a hint of convergent but overall a close match

```{r}
#| label: cem - Alliance Psychology
#| fig-height: 8
#| cache: true
#| eval: true
...
**Outcome 1:** ✔️ The trends are almost parallel, there was a convergent in April 2022 to June 2022 but these appear to have resolved throughout the rest of the time series

**Outcome 2:** ✔️ The trends are parallel

```{r}
#| label: cem - Livewell Southwest
#| fig-height: 8
#| cache: true
#| eval: true
...
```{r}
#| label: cem - Vitaminds West Essex
#| fig-height: 8
#| cache: true
#| eval: true
...
A total of 6 Talking Therapy services were matched with `r params$intervention_long_name` using *Coarsened Exact Matching (CEM)*. Of these, 3 exhibited pre-intervention trends that closely align with those of `r params$intervention_long_name` for both outcome measures.
...
Now we have matched `r params$intervention_long_name` with 6 other Talking Therapy services and assessed their likelihood of meeting the parallel trends assumption based on their pre-intervention data, let us review how the full time series looks for this group of services.
...
-   Each series is identified by its ODS code at the end of the data in Sep 2025.
...
-   `r params$intervention_short_name` had a larger-than-usual dip in Oct 2023 to 56%,

-   RTF61 increased to 100% in May 2024
...
-   T6G0K performance increased to 75% in January 2025.

-   `r params$ods_intervention` dropped to 25% in September 2023.
...
```{r}
#| label: cem - spaghette plot2 outcomes 1 and 2
#| fig-height: 6
#| cache: true
...
df_analysis2 <-
  ...
  dplyr::filter(
    !(ods_code == "RJ8" &
      calc_month %in% zoo::as.yearmon(c("Oct 2023", "Sep 2023"))),
    !(ods_code == "T6G0K" &
      calc_month %in% zoo::as.yearmon(c("Dec 2024"))),
    !(ods_code == "RTF61" &
      calc_month %in% zoo::as.yearmon(c("May 2024")))
  )
...
This table summarises the Difference-in-Differences analysis for `r params$intervention_long_name`'s interventions using the 3 controls identified using the Coarsened Exact Matching process and which exhibited parallel trends.

For outcome 1, `r params$intervention_long_name`'s observed rate is about **4.26% lower** than what the matched controls would predict, with a 95% confidence interval (-0.49% to 9.02%) that includes zero, indicating **no statistically reliable difference**.

For outcome 2, `r params$intervention_long_name`'s rate is about **5.04% higher** than expected, with a 95% confidence interval (2.41% to 7.68%) indicating **a statistically significant difference**.
...
# work out the expected number of people to benefit from the uplift
people_benefit_o1 <- floor(total_discharges * (4.26 / 100))
people_benefit_o2 <- floor(total_discharges * (5.04 / 100))
```
...
1.  **Parallel trend selection** - We inspected the 6 matched controls series visually and excluded two that did not follow `r params$intervention_long_name`'s pre-intervention trend.

2.  **Anomalous month removal** - For the remaining matched services we omitted a month for two services with extreme values, which yielded more stable performance across outcomes.
...
-   Outcome 1 changes to show significant effect, and the estimate becomes positive.
...
These sensitivity tests add new insight to the main findings.
...
-   **Outcome 1** The two models (CEM and keeping anomalous months) show no statistically significant effect but using all CEM controls model is statistically significant.
...
-   The {synthdid} package requires a balanced panel; every service in the donor pool must have data for every month from Jan 2022 through Sep 2025.
...
set.seed(12345)
.yearmon_period = zoo::as.yearmon(c("Jan 2022", "Sep 2025"))
...
`r params$intervention_long_name`'s orange line rises more sharply than the dotted counterfactual, indicating a positive impact of the interventions. The estimated gain is 3.3% above the expected value. The 95% confidence interval spans -4.7% to + 11.3%, meaning the true effect could be negative or positive. Consequently, the result is not statistically significant.
...
`r params$intervention_long_name`'s orange line rises much more sharply than the dotted counterfactual, indicating an effect in the post-intervention period. The estimated difference is 4.9% above the expected value. The 95% confidence interval spans -1.9% to +11.6%, meaning the true effect could be negative or positive. Consequently, the result is **not statistically significant**.
...
# work out the expected number of people to benefit from the uplift
people_benefit_o1 <- floor(total_discharges * (3.3 / 100))
people_benefit_o2 <- floor(total_discharges * (4.9 / 100))
```
...
Assuming the 4.9% gain for Outcome 2 is real, it translates to an additional `r people_benefit_o2` people achieving reliable per year (based on `r params$intervention_long_name`'s annual discharge volume of `r scales::comma(total_discharges)` from June 2024 to May 2025).
:::
...
`r params$intervention_long_name` performed 5.7% below the synthetic control expectation, however, the 95% confidence interval (-4.4% to 15.9%) includes zero so we cannot make any definitive claim.
...
`r params$intervention_long_name` performed 6.2% above the synthetic control expectation, and crucially, the 95% confidence interval (-0.7% to 13%) narrowly does not include zero, which means this means this finding is **not statistically significant**.
...
`r params$intervention_long_name`'s observed outcome was **3.6% higher** than the synthetic control expectation. Additionally, because the 95% confidence interval (3.5% to 3.8%) does not include zero, the result is **statistically significant**.
...
`r params$intervention_long_name`'s observed outcome was **5.9% higher** than the synthetic control expectation. However, the 95% confidence interval (5% to 6.8%) not includes zero, so the result is statistically significant effect.
...
The main model's estimates are not statistically significant, but the alternative CEM model reveal significant effects for the two outcome.

the alternative CEM model produces narrower confidence intervals, suggesting better model precision.

The sensitivity analysis suggest that the main model may be underpowered or imprecise. The choice of matching method materially affects the interpretation of treatment effects for Outcome 1 and Outcome 2 .

There is a moderate evidence of statistically significant positive effect on Outcome 1 and Outcome 2.Thus, The choice of matching method and variation may affect the result of both outcomes.
...
The chart highlights a **0.8 percentage point decrease in mutually agreed discharges** among individuals who received five or more treatment contacts in the post-intervention period, compared to the pre-intervention period. This shift was accompanied by a 4.2 percentage point increase in dishcarges initiated earlier than the care professional intended, and an 4.1 percentage point reduction in discharges grouped under 'Other codes'.
...
-   **Manual DiD** - `r params$intervention_long_name` was compared with 6 services selected through a coarsened exact matched (CEM) procedure
...
Across all three main methods, the DiD estimates for Outcome 1 are small positive effect and statistically insignificant (confidence intervals include zero)

There is no robust evidence of a treatment effect on Outcome 1, and findings are sensitive to the matching method used.

## Outcome 2

The two methods (PSM and CEM) show a positive DiD estimate for Outcome 2.

the PSM analysis yields a statistically significant result (confidence interval does not include zero), suggesting a 4.87% increase attributable to the intervention.

the CEM analysis yields a statistically significant result (confidence interval does not include zero), suggesting a 5.04% increase attributable to the intervention.

SynthDiD shows 4.9% point estimate but lacks precision (wide confidence interval).

There is **moderate evidence of a positive treatment effect on Outcome 2**, particularly when using PSM and CEM. This suggests the interventions may have improved the outcome, through results vary by method.
