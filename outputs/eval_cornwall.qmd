...
## Using all matching variables - Jan 2023

The dataset is cut off at January 2023, the month immediately before `r params$intervention_short_name` launched its first intervention. This pre-intervention slice will serve as the basis for the matching process.
...
## Propensity score matching - January 2023 
...
Outcome 2: ✔️ The trends are almost parallel, there is a single drop but otherwise looks good
...
#### Herefordshire and Worchestershire Health and Care NHS Trust RA1
...
Outcome 1: ❌ The control’s trend is at  mostly 80% throughout and is not reliable
...
Outcome 1: ✔️ The trends are almost parallel, there is a single drop but otherwise looks good
...
matches_parallel_ods <- c("W5N3Z", "RRE")
...
df_analysis <-
  ...
  dplyr::filter(
    ...
    ods_code %in% c(params$ods_intervention, c("W5N3Z", "RRE"))
  ) |>
  # simplify the data
...
-   Two control service, W5N3Z and RRE, exhibited an abnormal spike to 100% and 97% performance in outcome 1 in October 2023 and February 2024.

-   `r params$intervention_long_name` also experienced a similar, but smaller dip to 56% in June 2024.
...
-   W5N3Z fell to 0% in October 2023

-   RRE rise to 67% in February 2024

-   `r params$intervention_long_name` also experienced a dip to 25% in September 2023
...
df_analysis2 <-
  ...
  dplyr::filter(
    !(ods_code == params$ods_intervention &
      calc_month %in%
        zoo::as.yearmon(c("Jun 2024","Sep 2023"))),
    !(ods_code == "RRE" &
      calc_month %in% zoo::as.yearmon(c("Feb 2024"))),
    !(ods_code == "W5N3Z" & calc_month %in% zoo::as.yearmon(c("Oct 2023")))
  )
...
For outcome 1, Cornwall’s observed rate is about 4.4% higher than what the matched controls would predict, and the 95% confidence interval (0.5% to 8.2%) indicates the effect is statistically significant.


For outcome 2, Cornwall’s rate is about 5.53% higher than what the matched control would predict, and the 95% confidence interval (3.2% to 7.9%) indicates the effect is statistically significant.
