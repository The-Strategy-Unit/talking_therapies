...
subtitle: "Evaluation of interventions to improve adherence to therapy in *NHS Talking Therapies NELFT*"
author: "Craig Parylo"
date: today
...
`r params$intervention_long_name` discharges data is available from the beginning of 2022 with the most recent data point being September 2025.
...
-   `m2_rate` (M13 Proportion of discharges for people aged under 26 years at referral)
...
df_prep |>
  ...
  dplyr::filter(
    value %in% c(0L, 1L),
    !variable %in% c("m2_rate", "m14_rate") # already excluded from the above process
  ) |>
...
It is clear there are no covariates for `r params$intervention_long_name` which are likely to result in separation.
...
matching_vars <- setdiff(
  matching_vars,
  c("m2_rate", "m14_rate", "m17_rate")
)
t <- get_vif_score_table(matching_vars = matching_vars, df = df_prep)
t
```

**Next step:** remove `m7_rate` from the matching model.
...
matching_vars <- setdiff(
  matching_vars,
  c("m2_rate", "m14_rate", "m17_rate", "m7_rate")
)
t <- get_vif_score_table(matching_vars = matching_vars, df = df_prep)
t
```

**Next step:** drop `m3_rate` from the matching model.
...
matching_vars <- setdiff(
  matching_vars,
  c("m2_rate", "m14_rate", "m17_rate", "m7_rate", "m3_rate")
)
t <- get_vif_score_table(matching_vars = matching_vars, df = df_prep)
t
```
...
matching_vars <- setdiff(
  matching_vars,
  c("m2_rate", "m14_rate", "m17_rate", "m7_rate", "m3_rate", "m10_rate")
)
t <- get_vif_score_table(matching_vars = matching_vars, df = df_prep)
t
```

**Next step:** remove `m16_rate`from the matching model.
...
**Next step:** drop `m13_rate` from the matching model.
...
All matching variables now have VIF scores below 4, confirming they are suitable for propensity-score matching. The model has converged, so the matching process proceeds without issue; the only remaining warning relates to having a single treated service and can safely be ignored.
...
These results indicate that the matched services are closer, though not perfect, comparators for `r params$intervention_long_name`.

Following matching, three variables show improved balance between `r params$intervention_long_name` and the eight matched services, as evidenced by blue points being closer to zero than red points. However, one variable (M11) became notably more imbalanced post-matching.

Two variables fall within the 0.2 Standardised Mean Difference (SMD) threshold.
...
#### Freeflow Counselling Service (DA1)
...
#### Psychological Therapies Southampton Office (RDYDL)

Outcome 1: ❌ The trends are not parallel - they are divergent

Outcome 2: ❌ The trends are not parallel - they intersected around June 2023

```{r}
#| label: PSM - southampton
#| fig-height: 8
#| cache: true
#| eval: true
plot_list[[3]]
...
#### Croydon Psychological Therapies Service (Croydon IAPT) (RV5CJ)
...
#### Wandsworth IAPT (RQY12)

Outcome 1: ✔️ The trends are almost parallel

Outcome 2: ✔️ The trends are almost parallel, there is a hint of divergence but they are close

```{r}
#| label: PSM - wandsworth
#| fig-height: 8
#| cache: true
#| eval: true
plot_list[[5]]
...
#### South Tyneside and Sunderland NHS FT (R0B)
...
matches_parallel_ods <- c("RQY12", "RX4K3")
...
p <-
  plot_spaghetti_plot(
    ...
    str_subtitle = "RX4K3 displays abnormal results in April 2025",
...
One control service, RX4K3, exhibited abnormal peak to 100% in April 2025. This brief, but steep, increase would bias the post-implementation estimates upward for the control services. To avoid this bias, the high-performance point should be excluded from the dataset.
    ...
    .f = \(.x) {
      p <-
        compare_matches_preintervention(
          df = df,
          selected_ods = c(params$ods_intervention, .x),
          period_preintervention = zoo::as.yearmon(c("Jan 2022", "Aug 2024")),
          trendline = TRUE
        )
      return(p)
    }
...
A total of 2 Talking Therapy services were matched with `r params$intervention_long_name` using *Coarsened Exact Matching (CEM)*. Of these, none exhibited pre-intervention trends that closely align with those of `r params$intervention_long_name` for both outcome measures.
...
There is no suitable matches using CEM based analysis method for NHS Talking Therapies NELFT
...
`r params$intervention_long_name`'s orange line does not rise in line with the dotted counterfactual, indicating a negative impact of the interventions. The estimated change is 4.4% below the expected value. The 95% confidence interval spans -5.2% to + 13.9%, meaning the true effect could be negative or positive. Consequently, the result is **not statistically significant**.
...
`r params$intervention_long_name`'s orange line rises faster than the dotted counterfactual, indicating a higher-than-expected effect in the post-intervention period. The estimated difference is 3.9% above the expected value. The 95% confidence interval spans -14.2% to +6.5%, meaning the true effect could be negative or positive. Consequently, the result is **not statistically significant**.
...
`r params$intervention_long_name` performed 5.5% below the synthetic control expectation, however, the 95% confidence interval (-1.7% to 12.7%) includes zero so we cannot make any definitive claim.
...
`r params$intervention_long_name` performed -3.1% above the synthetic control expectation, yet the 95% confidence interval (-17.5% to 11.2%) includes zero so we cannot make any definitive claim.
...
saveRDS(
  object = did_meta,
  file = here::here("data", "project", "did_estimates", "nelft.Rds")
)
```
...
Only the PSM baseline shows a statistically significant increase in Outcome 1. CEM and synthdid estimates are small and not significant.
