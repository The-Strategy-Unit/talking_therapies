...
df_prep |>
  ...
  dplyr::filter(
    value %in% c(0L, 1L),
    !variable %in% c("m3_rate", "m12_rate", "m14_rate") # already excluded from the above process
  ) |>
...
matching_vars <- setdiff(
  matching_vars,
  c("m3_rate", "m12_rate", "m14_rate", "m11_rate", "m16_rate")
)
t <- get_vif_score_table(matching_vars = matching_vars, df = df_prep)
t
```

**Next step:** evaluate the impact of removing `m13_rate` from the matching model.
...
matching_vars <- setdiff(
  matching_vars,
  c("m3_rate", "m12_rate", "m14_rate", "m11_rate", "m16_rate", "m13_rate")
)
t <- get_vif_score_table(matching_vars = matching_vars, df = df_prep)
t
```
...
**Next step:** evaluate the impact of removing `m8_rate` from the matching model.
...
**Next step:** evaluate the impact of removing `m5_rate` from the matching model.
...
**Next step:** evaluate the impact of removing `m7_rate` from the matching model.
...
**Next step:** evaluate the impact of removing `m15_rate` from the matching model.
...
**Next step:** evaluate the impact of removing `o1_denom` from the matching model.
...
**Next step:** evaluate the impact of removing `m10_rate` from the matching model.
...
All matching variables now have VIF scores of 1, confirming they are suitable for propensity-score matching. The model has converged, so the matching process proceeds without issue.
...
## Propensity score matching - March 2024
...
Following matching, both variables show improved balance between `r params$intervention_long_name` and the eight matched services, as evidenced by blue points being closer to zero than red points.

Neither variable falls with the 0.2 Standardised Mean Difference (SMD) threshold after matching.
...
```{r}
#| label: PSM - centre for psychology
#| fig-height: 8
#| cache: true
...
#### Dorking Healthcare Limited (DHC) (NNE)
...
#### St John's Health Centre (RQY58)

Outcome 1: ✔️ The trends are almost parallel, despite the control service's data starting in late 2022

Outcome 2: ✔️ The trends are parallel, despite the control service's data starting in late 2022
...
```{r}
#| label: PSM - oxfordshire foundation
#| fig-height: 8
#| cache: true
plot_list[[6]]
```

#### Bootham Park Hospital (RX34F)
...
#### The Counselling Foundation NGR01

Outcome 1: ❌ The trends are not parallel, there is clear divergence

Outcome 2: ❌ The trends are not parallel - they are convergent

```{r}
#| label: PSM - counselling foundation
#| fig-height: 8
#| cache: true
plot_list[[8]]
```
...
A total of eight Talking Therapy services were matched with `r params$intervention_long_name` using *Propensity Score Matching (PSM)*. Of these, six had pre-intervention trends that visually match those of `r params$intervention_long_name`'s for both outcome measures.
...
matches_parallel_ods <- c("NNE", "RQY58", "RNUDV", "RXX1Y", "RNUDT", "RX34F")
...
This table summarises the Difference-in-Differences (DiD) analysis for `r params$intervention_long_name`'s interventions using the six control services identified using the Propensity Score Matching process and which exhibited parallel trends.

The DiD reveals small estimated increases for both outcomes:

-   Outcome 1 shows a DiD estimate of 0.75%, with a 95% confidence interval ranging from -1.61% to 3.11%

-   Outcome 2 has a DiD estimate of 1.07%, with a 95% confidence interval of -1.01% to 3.15%

Since neither confidence interval excludes zero, **these findings are not statistically significant**, indicating that the observed differences could be due to chance rahter than a true effect.
...
people_benefit_o1 <- floor(total_discharges * (0.75 / 100))
people_benefit_o2 <- floor(total_discharges * (1.07 / 100))
```

These findings indicate `r params$intervention_long_name`'s post-intervention performance exceeds the counterfactual for both outcomes, but the wide confidence intervals are too wide to support a definitive claim of effectiveness.

Assuming the 1.07% gain is real, it translates to an additional **`r people_benefit_o2` people** achieving reliable recovery per year (based on `r params$intervention_long_name`'s annual discharge volume of `r scales::comma(total_discharges)` from June 2024 to May 2025).
:::
...
1.  **Parallel trend selection** - We inspected the 8 matched control services visually and excluded two that did not follow `r params$intervention_long_name`'s pre-intervention trend.

Below we reassess the findings after relaxing this decision to determine whether the original conclusions hold or are sensitive to this analytical choice.
...
```{r}
#| label: psm - sensitivity - all 8 controls
#| cache: true
...
did_overall <-
  dplyr::bind_rows(
    did_overall,
    did |>
      dplyr::mutate(specification = "Alternative 1 - use all 8 controls")
  )
...
did_meta <-
  dplyr::bind_rows(
    did_meta,
    tibble::tibble(
      specification = "PSM alternative 1 - use all 8 controls",
      outcome = did$outcome,
      estimate = did$estimate,
      se = did$std.error
    )
...
The DiD analysis indicates modest estimated effects for both outcomes:

-   Outcome 1 shows a DiD estimate of -0.8%, with a 95% confidence interval ranging from -3.41% to 1.82%

-   Outcome 2 has a DiD estimate of 2.03%, with a 95% confidence interval of -0.43% to 4.48%

Since neither confidence interval excludes zero, these results are not statistically significant, suggesting that the observed differences may be due to random variation rather than a true effect.
...
*When all 8 matched services are used as controls*, the results reflect the main findings in that none of the estimates are statistically significant, as all confidence intervals include zero. Outcome 2 consistently shows a positive effect across both models, though not significant.

These results imply that while there may be small effects, **there is insufficient evidence to conclude a meaningful impact** under either model.
...
```{r}
#| label: cem - centre for psychology
#| fig-height: 8
#| cache: true
#| eval: true
...
#### Family Refugee Support Project (K7Y6R)

**Outcome 1:** ❌ The trends are not parallel - they intersected around July 2023

**Outcome 2:** ❌ The trends are not convergent - the control runs at 0% throughout
...
#### Bootham Park Hospital - Admin (RX34F)
...
A total of 8 Talking Therapy services were matched with `r params$intervention_long_name` using *Coarsened Exact Matching (CEM)*. Of these, 10 exhibited pre-intervention trends that closely align with those of `r params$intervention_long_name` for both outcome measures.
...
df_analysis2 <-
  ...
  dplyr::filter(
    !(ods_code == "ANV01" &
      calc_month %in% zoo::as.yearmon(c("Nov 2022", "Aug 2022")))
  )
...
did_overall <-
  dplyr::bind_rows(
    did_overall,
    did |>
      dplyr::mutate(specification = "Alternative 3 - use all CEM controls")
  )
...
did_meta <-
  dplyr::bind_rows(
    did_meta,
    tibble::tibble(
      specification = "CEM alternative 3 - use all CEM controls",
      outcome = did$outcome,
      estimate = did$estimate,
      se = did$std.error
    )
...
df_test <-
  ...
  interpolate_missing_month_outcomes_for_intervention_service(
    .ods_intervention = "ANV01"
  ) |>
  interpolate_missing_month_outcomes_for_intervention_service(
    .ods_intervention = "NGR01"
  ) |>
  interpolate_missing_month_outcomes_for_intervention_service(
    .ods_intervention = "NNE"
  ) |>
  interpolate_missing_month_outcomes_for_intervention_service(
    .ods_intervention = "RNUDT"
  ) |>
  interpolate_missing_month_outcomes_for_intervention_service(
    .ods_intervention = "RNUDV"
  ) |>
  interpolate_missing_month_outcomes_for_intervention_service(
    .ods_intervention = "RQY58"
  ) |>
  interpolate_missing_month_outcomes_for_intervention_service(
    .ods_intervention = "RX34F"
  ) |>
  interpolate_missing_month_outcomes_for_intervention_service(
    .ods_intervention = "RXX1Y"
  ) |>
  interpolate_missing_month_outcomes_for_intervention_service(
    .ods_intervention = "AM7"
  ) |>
  interpolate_missing_month_outcomes_for_intervention_service(
    .ods_intervention = "AMC03"
  ) |>
  interpolate_missing_month_outcomes_for_intervention_service(
    .ods_intervention = "K7Y6R"
  ) |>
  interpolate_missing_month_outcomes_for_intervention_service(
    .ods_intervention = "AMC03"
  )
...
# update again and again and again and again and again
did_overall |>
...
Overall, none of the estimates are statistically significant, as all 95% confidence intervals include zero. This suggest the intervention **does not have a robust or consistent effect** on either outcome and across different specifications.
...
2.  **Reliable recovery** - assessing the proportion of discharges that met the criteria for reliable recovery
...
The bar chart below displays `r params$intervention_long_name`'s data only. Due to insufficient data, referrals that ended earlier than the patient desired have been grouped under 'Other codes'. The chart reveals a **modest decrease of 1.4 percentage points in discharges occurring earlier than the therapist had planned** in the group who received 5+ treatment contacts, alongside small increases in discharges by mutual agreement (0.7 percentage points) and those classified under 'Other codes' (0.7 percentage points) during the post-intervention period compared to the pre-intervention period.
...
The chi-squared test indicates the observed changes are statistically significant for the **5+ treatment intensity group** (p = 0.049), while results for the **2-4 treatment intensity group** (p = 0.063) fall just above the conventional threshold for significance.

These findings suggest that the intervention had a measurable impact on reducing the proportion of referrals that ended earlier than intended by the therapist.
...
-   **Manual DiD** - `r params$intervention_long_name` was compared with 8 services selected through a propensity score matched (PSM) procedure
...
For the PSM, CEM and synthetic approaches we performed a series of sensitivity checks, for example, expanding the control pool and changing the intervention start date and using different matching methods.
