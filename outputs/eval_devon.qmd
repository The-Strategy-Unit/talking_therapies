...
-   **No robust, consistent effect** was found for either a) the proportion of discharged referrals receiving five treatmtent contacts, or b) the proportion of referrals receiving between 2 and 4 treatment contacts.

-  **Possible modest positive signal**: point-estimates are slightly above zero, but confidence intervals are wide and the results are not statistically conclusive.
...
did <-
  delegate_synthdid_analysis(
    ...
    labs_subtitle = "Devon performed 0.6% higher than expected, but the difference is not statistically significant"
  )
...
did <-
  delegate_synthdid_analysis(
    ...
    str_outcome = "o2_rate",
    labs_title = "Outcome 2: discharges with 2 to 4 treatment contacts",
    labs_subtitle = "Devon performed 0.4% better than expected, but the difference is not statistically significant"
  )
...
Devon's orange line falls more sharply than the dotted counterfactual, indicating a positive impact of the interventions. The estimated difference is 0.4% below the expected value. The 95% confidence interval spans -8% to + 7%, meaning the true effect could be negative or positive. Consequently, the result is not statistically significant.
...
Assuming the 0.6% gain for Outcome 1 is real, it translates to an additional **`r people_benefit` people** receiving the more intensive therapy per year (based on Devon's annual discharge volume of `r scales::comma(total_discharges)` from June 2024 to May 2025).
:::
...
# get a dataset that is ready for synthetic DiD using
.ods_controls = c("DA6", "GDA01", "NDC05", "NDC17", "RWK53")
...
did <-
  delegate_synthdid_analysis(
    ...
    str_outcome = "o1_rate",
    labs_title = "Outcome 1: discharges with 5+ treatment contacts",
    labs_subtitle = "Devon performed 2.4% worse than expected"
  )
...
Devon performs **2.4% lower** than the synthetic control expectation, and this difference is statistically significant. However, the synthetic control exhibits considerable variability both before and after the intervention, which adds uncertainty to the interpretation of the results.
...
did <-
  delegate_synthdid_analysis(
    ...
    str_outcome = "o2_rate",
    labs_title = "Outcome 2: discharges with 2 to 4 treatment contacts",
    labs_subtitle = "Devon performed 0.4% worse than expected"
  )
...
This analysis shows Devon performed **0.4% above** the synthetic control expectation. However, the synthetic control's high variability limits confidence in this modest effect.
...
did <-
  delegate_synthdid_analysis(
    ...
    str_outcome = "o1_rate",
    labs_title = "Outcome 1: discharges with 5+ treatment contacts",
    labs_subtitle = "Devon performed 2.8% better than expected, but the difference is not statistically significant"
  )
...
The headline figure - 2.8% higher performance - is misleading. In mid-2023 the synthetic control series falls sharply because two donor pool services experienced sudden drops. This dip pulls the synthetic control average down, making Devon's relative advantage appear larger than it truly is.
...
These anomalous spikes in mid-2023 also warp the post-intervention synthetic control series, just as they did for Outome 1.
...
The sensitivity analyses suggest that the main results are broadly credible, but they also highlight were the evidence is fragile.
...
*Propensity score matching (PSM)* - When Devon is compared with the services selected by the PSM algorithm, the direction of effect flips: Devon appears to perform **significantly worse** than its synthetic control. This result is dubious because the synthetic control series shows extreme variability, likely reflecting the poor qualitiy matches produced by the PSM step.
...
We estimated the impact of Devon's interventions using two complimentary DiD approaches:  

-   **Manual DiD** - Devon was compared with two services selected through a coarsened exact matching (CEM) procedure

-   **Synthetic control DiD** - Devon was contrasted with a synthetic counterfactual build from all available donor services.

For each approach we performed a series of sensitivity checks, for example, removing anomalous months, expanding the control pool, swapping CEM for PSM matches.

**No robust, consistent effect appears across the different specifications.** The few statistically significnat estimates are **highly sensitive** to modelling choices; once the analysis is made more conservative the significance disappears.

Because the evidence is not stable, any claim that the intervention produced a meaningful improvement, or deterioration, should be treated with caution.

**Possible signal** 

Both the manual DiD (point estimate ~ 4.9%) and the synthetic control DiD (point estimate 0.6%) point in the same direction, suggesting a modest positive effect. However, the confidence intervals are wide and overlap zero, so this 'hint' cannot be recorded as conclusive.
...
Two control services experienced abrupt shocks in mid-2023 which distorted the post-implementaton average performance and synthetic control trajectory, increased variability and weakened the credibility of the counterfactual.

**No single treatment date**

Devon's rollout was staggered across several components and spread over twelve moonths. As a result the "treated" period is diffuse, making it difficult to define a clean pre-/post-intervention split and to isolate the effect of any one component.

**Limited pre-intervention observations**

There are only 11 months of pre-treatment data versus 12 months of treatment and 18 months of post-treatment. Fewer pre-period points reduce confidence in the parallel-trend (or pre-fit) assumption and make the estimate more sensitive to short-term fluctations.
...
Take together, these limitations suggest that the estimated impacts should be interpreted with caution and highlight the need for a more rigorous, preferably experiemental, design in future evaluations.
